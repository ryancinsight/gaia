...
/// Thread-safe iterative AABB-overlap query over a flat-arena BVH using a SharedGhostToken.
pub(super) fn query_overlapping_shared<'brand>(
    node_aabbs: &[Aabb],
    node_kinds: &PermissionedArena<'brand, BvhNodeKind>,
    indices: &[usize],
    prim_aabbs: &[Aabb],
    ...
    while top > 0 {
        ...
        let kind = *node_kinds.get_shared(node_idx, token);
