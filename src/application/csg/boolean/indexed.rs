...
pub fn csg_boolean(
    ...
    if op == BooleanOp::Union {
        if let Some(union) = rectangular_prism_union(mesh_a, mesh_b) {
            return Ok(union);
        }
    }
...
/// combine into one rectangular prism.
///
/// The general arrangement path must retain coplanar subdivisions from an
/// intermediate result. Reusing that subdivision in a later Boolean can
/// misclassify coplanar fragments. A rectangular-prism union has a closed-form
/// boundary, so reconstructing its six planes avoids progressive topology
/// degradation while preserving the input region identity.
fn rectangular_prism_union(mesh_a: &IndexedMesh, mesh_b: &IndexedMesh) -> Option<IndexedMesh> {
    let bb_a = mesh_a.bounding_box();
    let bb_b = mesh_b.bounding_box();
