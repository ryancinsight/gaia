...
impl<T: Scalar> Tetrahedron<T> {
    ...
    pub fn new(mut v: [usize; 4], points: &[Point3<T>]) -> Self {
        ...
        // Since `cfd_mesh::predicates::orient_3d` negates `gp::orient3d` to maintain the right-hand rule,
        // a Shewchuk-positive orientation actually corresponds to `cfd_mesh::orient_3d.is_negative()`.
        // Therefore, we swap to fix the orientation if `cfd_mesh::orient_3d` evaluates as POSITIVE.
