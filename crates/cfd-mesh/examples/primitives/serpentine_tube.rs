...
use cfd_mesh::SerpentineTube;
use std::fs;
use std::io::BufWriter;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mesh = SerpentineTube::default().build()?;
    let edges = EdgeStore::from_face_store(&mesh.faces);
    let report = check_watertight(&mesh.vertices, &mesh.faces, &edges);
    println!(
        "SerpentineTube V={} F={} watertight={} vol={:.4} chi={:?}",
        mesh.vertices.len(),
        mesh.faces.len(),
    ...
    stl::write_binary_stl(
        &mut BufWriter::new(fs::File::create(out.join("serpentine_tube.stl"))?),
        &mesh.vertices,
        &mesh.faces,
    )?;
    Ok(())
