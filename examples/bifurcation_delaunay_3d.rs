...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let top_branch = CapsuleSdf::new(Point3::new(0.0, 0.0, 0.0), Point3::new(10.0, 5.0, 0.0), 1.5);
    ...
    for cell in mesh.cells() {
        if cell.vertex_ids.len() == 4 {
            let a = mesh
                .vertices
                .position(VertexId::from_usize(cell.vertex_ids[0]))
                .coords;
            let b = mesh
                .vertices
                .position(VertexId::from_usize(cell.vertex_ids[1]))
                .coords;
            let c = mesh
                .vertices
                .position(VertexId::from_usize(cell.vertex_ids[2]))
                .coords;
            let d = mesh
                .vertices
                .position(VertexId::from_usize(cell.vertex_ids[3]))
                .coords;
    ...
    // Watertight validation following the canonical gaia pattern.
