...
//! Run with:
//! ```sh
//! cargo run -p gaia --example csg_union
//! ```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut mesh = csg_boolean(BooleanOp::Union, &cube_a, &cube_b)?;
    ...
    let status = if vol_ok && align_mean_ok {
        "PASS"
    } else {
        "FAIL"
    };
